...
class M_home extends CI_Model
{
	...
	public function jml_ulasan($id)
	{
		return $this->db->query("SELECT COUNT(status) as jml FROM `riview` LEFT JOIN produk ON produk.id_produk=riview.id_produk WHERE status=1 AND `riview`.id_produk='" . $id . "'")->result();
	}
